...
def get_args():
    ...
    parser.add_argument('--fgLists',
                        type=list,
                        default=['fg_list.txt'],
                        help="training fore-ground images lists")
    ...
    parser.add_argument('--dataRatio',
                        type=list,
                        default=[1],
                        help="train bg:fg raio, eg. [100]")
    parser.add_argument('--saveDir', default='./ckpt', help='model save dir')
    parser.add_argument('--trainData',
                        default='human_matting_data',
                        help='train dataset name')

    parser.add_argument('--continue_train',
                        action='store_true',
                        default=False,
                        help='continue training the training')
    parser.add_argument('--pretrain',
                        action='store_true',
                        help='load pretrained model from t_net & m_net ')
    parser.add_argument('--without_gpu',
                        action='store_true',
                        default=False,
                        help='no use gpu')
    ...
    parser.add_argument('--print_iter',
                        type=int,
                        default=1000,
                        help='pring loss and save image')
...
def save_img(args, all_img, epoch, i=0):
    ...
    if args.train_phase == 'pre_train_t_net':
        ...
        trimap_cat = torch.cat((trimap_gt[0][0], trimap_fake.float()),
                               dim=-1)  # horizontal concate
    ...
    if args.train_phase == 'end_to_end':
        ...
        img_cat = torch.cat((img, trimap_gt, trimap_pre, alpha_gt, alpha_pre),
                            dim=-1)
        ...
        # save image
        cv2.imwrite(img_dir + '/e2e_{}_{}.png'.format(str(epoch), str(i)),
                    img_cat)
...
class Train_Log():
    ...
    def __init__(self, args):
        ...
        if self.args.pretrain:
            self.t_path = os.path.join(args.saveDir, 'pre_train_t_net', 'model',
                                       'ckpt_lastest.pth')
            self.m_path = os.path.join(args.saveDir, 'pre_train_m_net', 'model',
                                       'ckpt_lastest.pth')
    ...
    def save_model(self, model, epoch, save_as=False):
        if save_as:  # for args.save_epoch
            lastest_out_path = "{}/ckpt_{}.pth".format(self.save_dir_model,
                                                       epoch)
    ...
    for epoch in range(start_epoch, args.nEpochs + 1):
        ...
        for i, sample_batched in enumerate(trainloader):
            ...
            if args.train_phase == 'pre_train_t_net':
                ...
                img, trimap_gt, alpha_gt, bg, fg = img.to(device), trimap_gt.to(
                    device), alpha_gt.to(device), bg.to(device), fg.to(device)
                ...
                loss, L_alpha, L_composition = loss_f_M(img, alpha_pre,
                                                        alpha_gt, bg, fg,
                                                        trimap_gt)
